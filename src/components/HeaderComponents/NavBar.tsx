...
import { BookOpenCheck, Linkedin, Github, FileText, Home, Code, User } from 'lucide-react';
import './NavBar.css';
...
const NavBar: React.FC<Props> = ({ itemStyle, navStyle, items }) => {
  ...
  const getIcon = (label: string) => {
    switch (label.toLowerCase()) {
      ...
      case 'tech stack': return <Code size={18} />;
      case 'research': return <BookOpenCheck size={18} />;
      case 'projects': return <Home size={18} />;
  ...
  return (
    <nav className={`navbar-container ${navStyle}`}>
      ...
      {items.map((item, index) => (
        <a
          key={`${index}_nav`}
          className={`${itemStyle} navbar-item ${activeItem === item.label ? 'active' : ''}`}
